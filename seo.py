...
NON_AFFILIATION = ("Jev is a model by TypeSafe. Jev Router is independent, is not affiliated with "
                   "TypeSafe and does not offer Jev itself.")
...
DESCRIPTIONS: dict[str, str] = {
    ...
    "/docs": "API docs for Jev Router: concrete model routes, the jev-class and image-jev-class "
             "meta routes, curl and Python examples, and the on-demand hosting lifecycle.",
...
HOME_FAQ = [
    ...
    ("How does the jev-class route pick a model?",
     "It tries healthy text models in descending published JevBench score order; the next score "
     "is the fallback and the model ID breaks ties. The model that answered and its price are "
...
def register(app: FastAPI, page: Callable[..., HTMLResponse], models: Callable[[], dict[str, dict[str, Any]]],
    ...
    def models_table() -> str:
        rows = []
        for key, info in models().items():
            score = info.get("jevbench") or info.get("multimodal_benchmark")
            bench = f'{_e(score["score"])} ({_e(score["track"])})' if score else "—"
    ...
    example = ('curl https://jev-router.com/v1/systemone \\\n  -H "Authorization: Bearer jvr_…" \\\n'
               ...
               '  -d \'{"model":"jev-class","state":"Ticket: I was charged twice this month.",'
    ...
    def decision_model_api(request: Request):
        body = (f'<h1 style="font-size:52px">Decision model API</h1><p class="lead">Ask typed questions about some '
                ...
                f'Name a concrete model to pin it, or use <code>jev-class</code> to let the published score order '
    ...
    def jev_alternatives(request: Request):
        body = ('<h1 style="font-size:52px">Open Jev alternatives</h1><p class="lead">Jev-class decision models '
                ...
                '<h2>Choosing one</h2><p>Use <code>jev-class</code> if you want the best healthy text model by '
                'published score with automatic fallback. Pin a concrete model if you need a stable price, a '
                'specific licence, or image input. Dedicated GPU hosting, where available, lets you run an open '
                'model for yourself by the minute.</p>'
