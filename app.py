...
<pre tabindex="0">{esc(DOC_EXAMPLE)}</pre><h3>Response</h3><pre tabindex="0">{esc(RESPONSE_EXAMPLE)}</pre>
<h2>Images</h2><pre tabindex="0">{esc(IMAGE_EXAMPLE)}</pre>
<h2>OpenAI-compatible</h2><p><code>POST /v1/chat/completions</code> and <code>GET /v1/models</code>. Put typed questions in the extra top-level <code>questions</code> field; the chat messages become the state. The assistant message content is the JSON answers object, and the full decision is in <code>decision</code>. You can also send a complete decision request as JSON in the last user message.</p><pre tabindex="0">{esc(OPENAI_EXAMPLE)}</pre>
...
<h3>5. Credit and payment</h3><p>The service runs on prepaid credit bought through Stripe. Usage is charged per decision at the price shown for the model that answered. Credit does not expire, has no cash value and is not transferable. We never charge you automatically. Prices are in US dollars; {VAT_NOTE} Refunds: see the <a href="/refunds">refund policy</a>.</p>
...
async def create_key(request: Request):
    ...
    return page("API key created",f'<h1 style="font-size:52px">Copy your key</h1><p class="notice">This is the only time the full key is shown.</p><pre id="new-key" tabindex="0">{esc(raw)}</pre><a class="button" href="/dashboard">Done</a>',u)
