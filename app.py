...
def page(title: str, body: str, user=None) -> HTMLResponse:
    ...
    return HTMLResponse(f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Neutral infrastructure for explicit Jev-class decision model routing."><title>{esc(title)} · Jev Router</title><style>{CSS}</style></head><body><nav aria-label="Main navigation"><a class="brand" href="/">Jev Router</a><a href="/models-page">Models</a><a href="/docs">Docs</a><a href="/status">Status</a>{auth}</nav><main>{body}</main><footer><span>© 2026 productivity-boost.com Betriebs UG &amp; Co. KG</span><a href="/terms">Terms</a><a href="/privacy">Privacy</a><a href="/refunds">Refunds</a><a href="/impressum">Impressum</a></footer></body></html>''')
...
def models_page(request: Request):
    rows="".join(f'<div class="model"><div><strong>{esc(k)}</strong><br><span class="muted">{esc(v["provider"])}</span></div><div><span class="badge {v["status"]}">{esc(v["status"])}</span></div><div>{esc(", ".join(v["modalities"]))}</div><div>{esc(v["billing"])}</div></div>' for k,v in MODELS.items())
    return page("Models", '<h1 style="font-size:52px">Models</h1><p class="lead">Availability, capabilities, and prices. JevBench figures are context—not an automatic recommendation.</p><div class="model muted"><div>Model</div><div>Status</div><div>Input</div><div>Price</div></div>'+rows+'<p class="notice">BYOK means your provider key is forwarded for that request and never stored. Hosted Maisa djev is not proxied.</p>', current_user(request))
...
LEGAL={
"terms":("Terms of service","Jev Router is a prepaid, usage-based decision-model gateway. You must choose a model explicitly and use it lawfully. Third-party BYOK traffic is governed by that provider’s terms and carries no Jev Router markup. Self-hosted usage is deducted from prepaid credits at the published price. No SLA or warranty is provided. We may suspend abusive or unsafe traffic and enforce rate and spend limits."),
...
async def stripe_webhook(request:Request,stripe_signature:str|None=Header(None)):
    ...
    event=json.loads(payload); obj=event.get("data",{}).get("object",{})
    with dbconn() as db:
